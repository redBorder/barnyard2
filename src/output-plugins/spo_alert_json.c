...
static void AlertJSONInit(char *args) {
	...
	DEBUG_WRAP(DebugMessage(DEBUG_INIT,
	                        "Linking JSON functions to call lists...\n"););
...
rdkafka_add_attr_to_config(rd_kafka_conf_t *rk_conf,
...
static rd_kafka_conf_res_t
rdkafka_add_str_to_config(rd_kafka_conf_t *rk_conf,
                          rd_kafka_topic_conf_t *rkt_conf,
	...
	*val = '\0';
	...
	const rd_kafka_conf_res_t rc = rdkafka_add_attr_to_config(rk_conf, rkt_conf,
	        key, val, errstr, errstr_size);
	free(keyval);
	return rc;
}
...
static AlertJSONData *AlertJSONParseArgs(char *args) {
	...
	char *filename = NULL;
	int i;
	char *hostsListPath = NULL, *networksPath = NULL, *servicesPath = NULL,
	*protocolsPath = NULL, *vlansPath = NULL, *prioritiesPath = NULL;
#ifdef HAVE_GEOIP
	...
	for (i = 0; i < num_toks; i++) {
		...
		if ( !strncasecmp(tok, "filename=", strlen("filename=")) && !filename) {
			RB_IF_CLEAN(filename, filename = SnortStrdup(tok + strlen("filename=")),
			            "%s(%i) param setted twice\n", tok, i);
		} else if (!strncasecmp(tok, "params=", strlen("params=")) && !data->jsonargs) {
			RB_IF_CLEAN(data->jsonargs,
			            data->jsonargs = SnortStrdup(tok + strlen("params=")),
			            "%s(%i) param setted twice\n", tok, i);
		} else if (!strncasecmp(tok, KAFKA_PROT, strlen(KAFKA_PROT)) && !kafka_str) {
#ifdef HAVE_LIBRDKAFKA
			RB_IF_CLEAN(kafka_str, kafka_str = SnortStrdup(tok),
			            "%s(%i) param setted twice\n", tok, i);
#else
			FatalError("alert_json: This barnyard was build with no librdkafka support.");
#endif
		} else if (!strncasecmp(tok, HTTP_PROT, strlen(HTTP_PROT))
		           || !strncasecmp(tok, HTTPS_PROT, strlen(HTTP_PROT))) {
#ifdef HAVE_LIBRBHTTP
			RB_IF_CLEAN(data->http.url, data->http.url = SnortStrdup(tok),
			            "%s(%i) param setted twice\n", tok, i);
#else
			FatalError("alert_json: This barnyard was build with no http support.");
#endif
		} else if ( !strncasecmp(tok, "default", strlen("default"))
		            && !data->jsonargs) {
			RB_IF_CLEAN(data->jsonargs, data->jsonargs = SnortStrdup(DEFAULT_JSON),
			            "%s(%i) param setted twice\n", tok, i);
		} else if (!strncasecmp(tok, "enrich_with=", strlen("enrich_with="))) {
			RB_IF_CLEAN(data->enrich_with,
			            data->enrich_with = SnortStrdup(tok + strlen("enrich_with=")),
			            "%s(%i) param setted twice\n", tok, i);
			...
			RB_IF_CLEAN(hostsListPath, hostsListPath = SnortStrdup(tok + strlen("hosts=")),
			            "%s(%i) param setted twice.\n", tok, i);
		} else if (!strncasecmp(tok, "networks=", strlen("networks="))) {
			RB_IF_CLEAN(networksPath, networksPath = SnortStrdup(tok + strlen("networks=")),
			            "%s(%i) param setted twice.\n", tok, i);
		} else if (!strncasecmp(tok, "services=", strlen("services="))) {
			RB_IF_CLEAN(servicesPath, servicesPath = SnortStrdup(tok + strlen("services=")),
			            "%s(%i) param setted twice.\n", tok, i);
		} else if (!strncasecmp(tok, "protocols=", strlen("protocols="))) {
			RB_IF_CLEAN(protocolsPath,
			            protocolsPath = SnortStrdup(tok + strlen("protocols=")),
			            "%s(%i) param setted twice.\n", tok, i);
		} else if (!strncasecmp(tok, "priorities=", strlen("priorities="))) {
			RB_IF_CLEAN(prioritiesPath,
			            prioritiesPath = SnortStrdup(tok + strlen("priorities=")),
			            "%s(%i) param setted twice.\n", tok, i);
		} else if (!strncasecmp(tok, "vlans=", strlen("vlans"))) {
			RB_IF_CLEAN(vlansPath, vlansPath = SnortStrdup(tok + strlen("vlans=")),
			            "%s(%i) param setted twice.\n", tok, i);
		} else if (!strncasecmp(tok, "rdkafka.", strlen("rdkafka."))) {
#if HAVE_LIBRDKAFKA
			char errstr[512];

			const rd_kafka_conf_res_t rc = rdkafka_add_str_to_config(data->kafka.rk_conf,
			        data->kafka.rkt_conf, tok + strlen("rdkafka."), errstr, sizeof(errstr));
			...
			if (!strncmp(tok, "http.max_connections=", strlen("http.max_connections="))) {
				data->http.max_connections = strtol(tok + strlen("http.max_connections="), &end,
				                                    0);
			} else if (!strncmp(tok, "http.max_queued_messages=",
			                    strlen("http.max_queued_messages="))) {
				data->http.max_queued_messages = strtol(tok +
				                                        strlen("http.max_queued_messages="), &end, 0);
			...
			RB_IF_CLEAN(eth_vendors_path,
			            eth_vendors_path = SnortStrdup(tok + strlen("eth_vendors=")),
			            "%s(%i) param setted twice.\n", tok, i);
		}
#ifdef HAVE_GEOIP
		else if (!strncasecmp(tok, "geoip=", strlen("geoip="))) {
			RB_IF_CLEAN(geoIP_path, geoIP_path = SnortStrdup(tok + strlen("geoip=")),
			            "%s(%i) param setted twice.\n", tok, i);
		} else if (!strncasecmp(tok, "geoip_org=", strlen("geoip_org="))) {
			RB_IF_CLEAN(geoIP_org_path,
			            geoIP_org_path = SnortStrdup(tok + strlen("geoip_org=")),
			            "%s(%i) param setted twice.\n", tok, i);
		}
#ifdef SUP_IP6
		else if (!strncasecmp(tok, "geoip6=", strlen("geoip6="))) {
			RB_IF_CLEAN(geoIP6_path, geoIP6_path = SnortStrdup(tok + strlen("geoip6=")),
			            "%s(%i) param setted twice.\n", tok, i);
		} else if (!strncasecmp(tok, "geoip6_org=", strlen("geoip6_org="))) {
			RB_IF_CLEAN(geoIP6_org_path,
			            geoIP6_org_path = SnortStrdup(tok + strlen("geoip6_org=")),
			            "%s(%i) param setted twice.\n", tok, i);
		}
#endif /* SUP_IP6 */
#endif /* HAVE_GEOIP */
	...
	if ( !filename ) filename = ProcessFileOption(barnyard2_conf_for_parsing,
		        DEFAULT_FILE);
	...
	if (geoIP_path) {
		...
		if (data->gi == NULL)
			...
			DEBUG_WRAP(DebugMessage(DEBUG_INIT,
			                        "alert_json: Success opening geoip database: %s\n", geoIP_path););
	} else {
		DEBUG_WRAP(DebugMessage(DEBUG_INIT,
		                        "alert_json: No geoip database specified.\n"););
	}
	...
	if (geoIP_org_path) {
		...
		DEBUG_WRAP(DebugMessage(DEBUG_INIT,
		                        "alert_json: No geoip organization database specified.\n"););
	}
...
static void AlertJsonKafkaDelayedInit (AlertJSONData *this) {
	...
	this->kafka.rk = rd_kafka_new(RD_KAFKA_PRODUCER, this->kafka.rk_conf, errstr,
	                              sizeof(errstr));
	...
	this->kafka.rkt = rd_kafka_topic_new(this->kafka.rk, this->kafka.topic,
	                                     this->kafka.rkt_conf);
...
static inline char *itoa10(uint64_t value, char *result,
                           const size_t bufsize) {return _itoa(value, result, 10, bufsize);}
...
static inline void printHWaddr(struct printbuf *pbuf, const uint8_t *addr,
...
static int extract_ip0(sfip_t *ip, const void *_event, uint32_t event_type,
                       Packet *p, int srcdst_req) {
	if (!(srcdst_req == SRC_REQ || srcdst_req == DST_REQ)) {
		ErrorMessage("extract_ip_from_packet called with no valid direction.");
		return SFIP_FAILURE;
	}
		...
		// Share the same structure until src/dst ip included
	{
		int rc;
		if (srcdst_req == SRC_REQ) {
			rc = sfip_set_raw(ip, ((Unified2IDSEventIPv6 *)_event)->ip_source.s6_addr,
			                  AF_INET6);
		} else {
			rc = sfip_set_raw(ip, ((Unified2IDSEventIPv6 *)_event)->ip_destination.s6_addr,
			                  AF_INET6);
		}
		if (p && rc != SFIP_SUCCESS)
			return extract_ip_from_packet(ip, p, srcdst_req);
...
static uint8_t extract_proto(const void *_event, uint32_t event_type,
                             Packet *p) {
	switch (event_type) {
	case UNIFIED2_IDS_EVENT:
	case UNIFIED2_IDS_EVENT_VLAN:
...
static uint16_t extract_port0(const void *_event, uint32_t event_type,
                              Packet *p, int srcdst_req) {
	if (!(srcdst_req == SRC_REQ || srcdst_req == DST_REQ)) {
...
static uint16_t extract_icmp_type(const void *_event, uint32_t event_type,
                                  Packet *p, int *okp) {
	const uint8_t proto = extract_proto(_event, event_type, p);
...
static uint16_t extract_icmp_code(const void *_event, uint32_t event_type,
                                  Packet *p, int *okp) {
	const uint8_t proto = extract_proto(_event, event_type, p);
	if (!(proto == IPPROTO_ICMP)) {
		if (okp)
...
static uint16_t extract_vlan_id(const void *_event, uint32_t event_type,
                                Packet *p, int *okp) {
	switch (event_type) {
	case UNIFIED2_IDS_EVENT_VLAN:
		// Share the same structure until ports included
...
static const char *extract_country0(AlertJSONData *jsonData, const sfip_t *ip,
                                    int format) {
...
static int printElementExtraDataBlob(AlertJSONTemplateElement *templateElement,
	...
	case SHA256:
		if (event_info == EVENT_INFO_FILE_SHA256) {
			...
			len = (int) (ntohl(U2ExtraData->blob_length) - sizeof(U2ExtraData->data_type) -
			             sizeof(U2ExtraData->blob_length));
...
static int printElementExtraData(void *event, uint32_t event_type,
	...
	switch (event_type) {
	case UNIFIED2_IDS_EVENT:
		extra_data_cache = &(((Unified2IDSEvent_legacy_WithPED *)(
		                          event))->extra_data_cache);
		break;
	...
	case UNIFIED2_IDS_EVENT_VLAN:
		...
		break;
	case UNIFIED2_IDS_EVENT_IPV6:
		extra_data_cache = &(((Unified2IDSEventIPv6_legacy_WithPED *)(
		                          event))->extra_data_cache);
		break;
	case UNIFIED2_IDS_EVENT_IPV6_MPLS:
	case UNIFIED2_IDS_EVENT_IPV6_VLAN:
		extra_data_cache = &(((Unified2IDSEventIPv6_WithPED *)(
		                          event))->extra_data_cache);
		break;
	default:
	...
	TAILQ_FOREACH(edrnCurrent, extra_data_cache, entry) {
		U2ExtraData = (Unified2ExtraData *)(((Unified2ExtraDataHdr *)edrnCurrent->data)
		                                    + 1);
...
static int printElementWithTemplate(Packet *p, void *event, uint32_t event_type,
	...
	if (NULL == templateElement)
		FatalError("TemplateElement was not setted (File %s line %d)\n.", __FILE__,
		           __LINE__);
#endif
	switch (templateElement->id) {
	case TIMESTAMP:
		printbuf_memappend_fast_str(printbuf,
		                            itoa10(ntohl(((Unified2EventCommon *)event)->event_second), buf, bufLen));
		break;
	case SENSOR_ID_SNORT:
		printbuf_memappend_fast_str(printbuf,
		                            event ? itoa10(ntohl(((Unified2EventCommon *)event)->sensor_id), buf,
		                                    bufLen) : templateElement->defaultValue);
		break;
	case ACTION:
	...
	case SIG_GENERATOR:
		if (event != NULL)
			printbuf_memappend_fast_str(printbuf,
			                            itoa10(ntohl(((Unified2EventCommon *)event)->generator_id), buf, bufLen));
		break;
	case SIG_ID:
		if (event != NULL)
			printbuf_memappend_fast_str(printbuf,
			                            itoa10(ntohl(((Unified2EventCommon *)event)->signature_id), buf, bufLen));
		break;
	case SIG_REV:
		if (event != NULL)
			printbuf_memappend_fast_str(printbuf,
			                            itoa10(ntohl(((Unified2EventCommon *)event)->signature_revision), buf, bufLen));
		break;
	case PRIORITY:
		if (event != NULL) {
			...
			printbuf_memappend_fast_str(printbuf,
			                            prio_name ? prio_name : templateElement->defaultValue);
		}
		break;
	case CLASSIFICATION:
		if (event != NULL) {
			uint32_t classification_id = ntohl(((Unified2EventCommon *)
			                                    event)->classification_id);
			const ClassType *cn = ClassTypeLookupById(barnyard2_conf, classification_id);
			printbuf_memappend_fast_str(printbuf,
			                            cn ? cn->name : templateElement->defaultValue);
	...
	case PROTO: {
		...
		Number_str_assoc *service_name_asoc = SearchNumberStr(proto,
		        jsonData->protocols);
	...
	case ETHSRC_VENDOR:
		if (p && p->eh && jsonData->eth_vendors_db) {
			const char *vendor = rb_find_mac_vendor(HWADDR_vectoi(p->eh->ether_src),
			                                        jsonData->eth_vendors_db);
			if (vendor)
	...
	case ETHDST_VENDOR:
		if (p && p->eh && jsonData->eth_vendors_db) {
			const char *vendor = rb_find_mac_vendor(HWADDR_vectoi(p->eh->ether_dst),
			                                        jsonData->eth_vendors_db);
			if (vendor)
				printbuf_memappend_fast_str(printbuf, vendor);
		}
	...
	case ETHTYPE:
		if (p && p->eh) {
			printbuf_memappend_fast_str(printbuf, itoa10(ntohs(p->eh->ether_type), buf,
			                            bufLen));
		}
		break;
	...
	case UDPLENGTH:
		if (p && p->udph) {
			printbuf_memappend_fast_str(printbuf, itoa10(ntohs(p->udph->uh_len), buf,
			                            bufLen));
		}
		break;
	case ETHLENGTH:
	...
	case DST_NAME: {
		...
		const char *ip_name = ip_str_node ? ip_str_node->human_readable_str :
		sfip_to_str(&ip);
	...
	case ICMPID:
		if (p && p->icmph)
			printbuf_memappend_fast_str(printbuf, itoa10(ntohs(p->icmph->s_icmp_id), buf,
			                            bufLen));
		break;
	case ICMPSEQ:
		if (p && p->icmph) {
			...
			*/
			printbuf_memappend_fast_str(printbuf, itoa10(ntohs(p->icmph->s_icmp_seq), buf,
			                            bufLen));
		}
		break;
	case TTL:
	...
	case ID:
		if (p && IPH_IS_VALID(p))
			printbuf_memappend_fast_str(printbuf,
			                            itoa10(IS_IP6(p) ? ntohl(GET_IPH_ID(p)) : ntohs((u_int16_t)GET_IPH_ID(p)), buf,
			                                   bufLen));
		break;
	case IPLEN:
		if (p && IPH_IS_VALID(p))
			printbuf_memappend_fast_str(printbuf, itoa10(ntohs(GET_IPH_LEN(p)), buf,
			                            bufLen));
		break;
	case IPLEN_RANGE:
		if (p && IPH_IS_VALID(p)) {
	...
	case TCPSEQ:
		if (p && p->tcph) {
			...
			printbuf_memappend_fast_str(printbuf, itoa10(ntohl(p->tcph->th_seq), buf,
			                            bufLen));
		}
		break;
	case TCPACK:
		if (p && p->tcph) {
			...
			printbuf_memappend_fast_str(printbuf, itoa10(ntohl(p->tcph->th_ack), buf,
			                            bufLen));
		}
		break;
	case TCPLEN:
		if (p && p->tcph) {
			printbuf_memappend_fast_str(printbuf, itoa10(TCP_OFFSET(p->tcph) << 2, buf,
			                            bufLen));
		}
		break;
	case TCPWINDOW:
		if (p && p->tcph) {
			...
			printbuf_memappend_fast_str(printbuf, itoa10(ntohs(p->tcph->th_win), buf,
			                            bufLen));
		}
		break;
	case TCPFLAGS:
...
static void RealAlertJSON(Packet *p, void *event, uint32_t event_type,
                          AlertJSONData *jsonData) {
	TemplateElementsList *iter;

	RefcntPrintbuf *rprintbuf = calloc(1, sizeof(*rprintbuf));
	if (NULL == rprintbuf) {
		ErrorMessage("alert_json, (%s:%d): Couldn't allocate (out of memory?)",
	...
	for (iter = jsonData->outputTemplate; iter; iter = iter->next) {
		...
		if (iter->templateElement->printFormat == stringFormat)
			printbuf_memappend_fast_str(printbuf, "\"");
		const int writed = printElementWithTemplate(p, event, event_type, jsonData,
		        iter->templateElement, printbuf);
		if (iter->templateElement->printFormat == stringFormat)
			printbuf_memappend_fast_str(printbuf, "\"");
	...
	if (jsonData->kafka.rkt) {
		const int produce_rc = rd_kafka_produce(jsonData->kafka.rkt,
